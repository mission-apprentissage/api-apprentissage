...
export async function checkDocumentationSync() {
  ...
  for (const id of Object.keys(apiOperations)) {
    ...
    const d = compareOperationObjectsStructure({ name: "lba", op: lbaOperation.operation }, { name: "api", op: apiOperation?.operation })
  ...
  const delta = structureDiff({ name: "expected", value: expectedDocumentationDelta }, { name: "actual", value: result })
