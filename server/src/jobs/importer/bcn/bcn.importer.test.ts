...
describe("runBcnImporter", () => {
  ...
  it("should import Bcn sources", async () => {
    ...
    expect(data.map((datum) => ({ ...datum, _id: "ObjectId" })).filter((datum) => datum.source === "N_FORMATION_DIPLOME")).toMatchSnapshot()
    expect(data.map((datum) => ({ ...datum, _id: "ObjectId" })).filter((datum) => datum.source === "N_FORMATION_DIPLOME_ENQUETE_51")).toMatchSnapshot()
    expect(data.map((datum) => ({ ...datum, _id: "ObjectId" })).filter((datum) => datum.source === "N_NIVEAU_FORMATION_DIPLOME")).toMatchSnapshot()
    expect(data.map((datum) => ({ ...datum, _id: "ObjectId" })).filter((datum) => datum.source === "V_FORMATION_DIPLOME")).toMatchSnapshot()
