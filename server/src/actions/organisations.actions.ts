...
export async function deleteOrganisation(organisationIdToDelete: ObjectId) {
  const organisation = await getDbCollection("organisations").findOne({ _id: organisationIdToDelete }, { projection: { nom: 1 } })
...
function removeOrganisationFromUsers(organisationName: string) {
  return getDbCollection("users").updateMany({ organisation: organisationName }, { $set: { organisation: null, updated_at: new Date() } })
}
