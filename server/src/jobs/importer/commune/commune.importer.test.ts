...
describe("runCommuneImporter", () => {
  ...
  it("should import initial communes", async () => {
    ...
    vi.mocked(fetchGeoRegion).mockImplementation(async (code: string) => sourceRegionExtendedFixtures.find((r) => r.code === code)!)
    vi.mocked(fetchGeoDepartements).mockImplementation(async (codeRegion: string) => sourceDepartementFixtures[codeRegion as keyof typeof sourceDepartementFixtures])
    vi.mocked(fetchGeoCommunes).mockImplementation(async (codeDepartement: string) => sourceCommuneFixtures[codeDepartement as keyof typeof sourceCommuneFixtures])
    ...
    expect(communes.map(({ _id, ...rest }) => rest).toSorted((a, b) => a.code.insee.localeCompare(b.code.insee))).toMatchSnapshot()
