...
function applySearch({ uai, siret }: OrganismeSearchQuery, organismeReferentiel: ISourceReferentiel): IRechercheOrganismeResultat {
  return {
    status: {
      ...
      siret:
        siret === null
          ...
          : {
              ...
              son_formateur: organismeReferentiel.data.relations?.some((relation) => relation.siret === siret && relation.type === "responsable->formateur") ?? false,
              son_responsable: organismeReferentiel.data.relations?.some((relation) => relation.siret === siret && relation.type === "formateur->responsable") ?? false,
            },
    },
    organisme: {
      identifiant: {
...
export async function searchOrganismeMetadata({ uai, siret }: OrganismeSearchQuery): Promise<IRechercheOrganismeResponse["metadata"]> {
...
export async function searchOrganisme({ uai, siret }: OrganismeSearchQuery): Promise<Omit<IRechercheOrganismeResponse, "metadata">> {
  if (!uai && !siret) {
    return {
      resultat: null,
      candidats: [],
  ...
  const organismes = organismeReferentiels.map((organismeReferentiel) => applySearch({ uai, siret }, organismeReferentiel))
  ...
  const bySiretAndUai = organismes.find((o) => o.correspondances.uai?.lui_meme && o.correspondances.siret?.lui_meme) ?? null
  ...
  const byUaiLieu = organismes.filter((o) => o.correspondances.uai?.son_lieu && o.status.ouvert && o.status.validation_uai)
  ...
  if (byUaiLieu.length === 1) {
    return buildResponse(byUaiLieu[0].correspondances.uai?.lui_meme || !byUaiLieu[0].correspondances.siret?.son_responsable ? byUaiLieu[0] : bySiret[0])
  }
