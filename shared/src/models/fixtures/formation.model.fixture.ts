...
export type IFormationFixtureInput = Partial<
  Omit<IFormationInternal, "formateur" | "responsable" | "certification" | "lieu"> & {
    ...
    lieu: Partial<IFormationInternal["lieu"]>;
  }
>;

export function generateFormationFixture(data?: IFormationFixtureInput): IFormationInternal {
  const { formateur, responsable, certification, lieu, ...rest } = data ?? {};
  ...
  return {
    ...
    formateur: {
      organisme: generateOrganismeFixture(formateur?.organisme),
      connu: formateur?.connu ?? true,
    },
    responsable: {
      organisme: generateOrganismeFixture(responsable?.organisme),
      connu: responsable?.connu ?? true,
    },
    certification: {
      valeur: generateCertificationFixture(certification?.valeur),
      connue: certification?.connue ?? true,
    },
    lieu: {
      adresse: {
