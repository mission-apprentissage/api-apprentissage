...
cat "$SEED_ARCHIVE" | docker compose -f "/opt/app/docker-compose.preview-system.yml" exec -iT mongodb mongorestore --archive --nsFrom="{{database_name}}.*" --nsTo="$TARGET_DB.*" --drop --numInsertionWorkersPerCollection=2 -j=2 --gzip "mongodb://__system:{{MONGODB_KEYFILE}}@localhost:27017/?authSource=local&directConnection=true"
