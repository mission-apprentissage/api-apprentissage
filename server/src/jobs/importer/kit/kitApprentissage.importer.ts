...
import { buildKitApprentissageEntry, buildKitApprentissageOp, getVersionNumber } from "./builder/kit_apprentissage.builder.js"
...
async function importKitApprentissageSourceCsv(filename: string): Promise<void> {
  try {
    ...
    throw withCause(internal("import.kit_apprentissage: unable to importKitApprentissageSourceCsv", { filename }), error)
  }
}
...
function getExcelParserSpec(): ExcelParseSpec {
  return [
    ...
    {
      type: "ignore",
      nameMatchers: [/Présentation Kit/i, /Corrections Kit/i, /Alerte CFD CS/i, /Évolutions Kit/i, /Evolutions Kit/i, /^Feuil1$/i],
    },
  ]
}
...
async function importKitApprentissageSourceXlsx(filename: string): Promise<void> {
  try {
    ...
    await pipeline(
      Duplex.from(parseExcelFileStream(createReadStream(getStaticFilePath(`kit_apprentissage/${filename}`)), getExcelParserSpec())),
      new Transform({
        objectMode: true,
        async transform(row: ExcelParsedRow, _encoding, callback) {
          try {
            ...
            callback(withCause(internal("import.kit_apprentissage: error when parsing row", { row, filename, version }), error))
          }
        },
      }),
      ...createBuildBulkOpAndWriteStreams()
    ...
    throw withCause(internal("import.kit_apprentissage: unable to importKitApprentissageSourceXlsx", { filename }), error)
  }
}
...
export async function getKitApprentissageImporterStatus(): Promise<ImportStatus> {
  const [lastImport, lastSuccess] = await Promise.all([
    ...
    await getDbCollection("import.meta").findOne({ type: "kit_apprentissage", status: "done" }, { sort: { import_date: -1 } }),
  ])
