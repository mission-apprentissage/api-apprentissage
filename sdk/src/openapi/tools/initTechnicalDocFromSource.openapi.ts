import type { ContentObject, OperationObject, ParameterObject, ReferenceObject, ResponsesObject, SchemaObject } from "openapi3-ts/oas31"

import type { DocRoute, DocTechnicalField } from "../../docs/types.js"

export function initRouteTechnicalDocFromSource(operation: OperationObject, sourceLang: "en" | "fr"): { operation: OperationObject; doc: DocRoute } {
  const { tags: _tags, security: _security, summary, description, parameters, requestBody, responses, ...rest } = operation
...
function initContentObjectTechnicalDoc(source: ContentObject, sourceLang: "en" | "fr"): { content: ContentObject; doc: DocTechnicalField | undefined } {
...
export function initModelTechnicalDocFromSource(
  ...
  const { description, allOf, oneOf, anyOf, not, items, properties, additionalProperties, propertyNames, prefixItems, examples, example, ...rest } = source
