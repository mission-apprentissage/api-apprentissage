...
function getTextOpenAPI<T extends OpenApiText | null | undefined>(value: T, lang: "fr" | "en" | null): T extends null | undefined ? null : string {
  if (value == null) {
...
function getTextOpenAPIArray<T extends OpenApiText[] | null | undefined>(value: T, lang: "fr" | "en" | null): T extends null | undefined ? null : string {
...
function pickPropertiesOpenAPI<T extends Record<string, SchemaObject>, K extends keyof T>(source: T, props: K[]): Record<K, SchemaObject> {
  return props.reduce(
    (acc, prop) => {
...
function addSchemaDocList<T extends SchemaObject | ReferenceObject>(schemas: ReadonlyArray<T>, docs: DocTechnicalField[], lang: "en" | "fr" | null, path: string[]): T[] {
...
function addSchemaDoc<T extends SchemaObject | ReferenceObject | undefined>(schema: T, doc: DocTechnicalField | undefined, lang: "en" | "fr" | null, path: string[]): T {
  ...
  if (output.properties && docProperties) {
    ...
    if (missingProps.length > 0) {
      throw new Error(`Propriété de documentation inconnu: ${missingProps.map((p) => `${path.join(".")}.${p}`).join(", ")}`)
    }
  ...
  if (output.allOf && doc.allOf) {
    output.allOf = typeof output.allOf === "boolean" ? output.allOf : addSchemaDocList(output.allOf, doc.allOf, lang, [...path, "allOf"])
  }
  if (output.oneOf && doc.oneOf) {
    output.oneOf = typeof output.oneOf === "boolean" ? output.oneOf : addSchemaDocList(output.oneOf, doc.oneOf, lang, [...path, "oneOf"])
  }
  if (output.anyOf && doc.anyOf) {
    output.anyOf = typeof output.anyOf === "boolean" ? output.anyOf : addSchemaDocList(output.anyOf, doc.anyOf, lang, [...path, "anyOf"])
  }
  ...
  if (typeof output.additionalProperties === "object" && doc.additionalProperties) {
    output.additionalProperties = addSchemaDoc(output.additionalProperties, doc.additionalProperties, lang, [...path, "additionalProperties"])
  }
  ...
  if (output.prefixItems && doc.prefixItems) {
    output.prefixItems = typeof output.prefixItems === "boolean" ? output.prefixItems : addSchemaDocList(output.prefixItems, doc.prefixItems, lang, [...path, "prefixItems"])
  }

  return output as T
}

function addContentObjectDoc(content: ContentObject, doc: DocTechnicalField | undefined, lang: "en" | "fr" | null): ContentObject {
  if (!doc) {
    return content
  }
...
export { addOperationDoc, addSchemaDoc, getTextOpenAPI, getTextOpenAPIArray, pickPropertiesOpenAPI }
