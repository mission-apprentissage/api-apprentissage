...
import { buildCertificationIntitule, validateNiveauFormationDiplomeToInterministerielRule } from "./certification.intitule.builder.js"
...
describe("validateNiveauFormationDiplomeToInterministerielRule", () => {
  ...
  describe("when NIVEAU_INTERMINISTERIEL is always the first character of NIVEAU_FORMATION_DIPLOME", () => {
    beforeEach(async () => {
      ...
      await getDbCollection("source.bcn").insertMany([generateSourceBcn_V_FormationDiplomeFixture(), generateSourceBcn_N51_FormationDiplomeFixture()])
    })
  ...
  describe("when NIVEAU_INTERMINISTERIEL is not the first character of NIVEAU_FORMATION_DIPLOME", () => {
    ...
    it("should rejects", async () => {
      await expect(validateNiveauFormationDiplomeToInterministerielRule()).rejects.toThrow("import.certifications: invalid niveau formation diplome to interministeriel rule")
    })
  })
})
