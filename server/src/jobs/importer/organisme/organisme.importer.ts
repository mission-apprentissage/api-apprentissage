...
async function getLatestImportMeta(): Promise<IImportMetaOrganismes | null> {
  const importMeta = await getDbCollection("import.meta").findOne<IImportMetaOrganismes>({ type: "organismes", status: "done" }, { sort: { import_date: -1 } })
...
async function buildOrganismeUpdateOperation(chunk: ISourceReferentiel, importMeta: IImportMetaOrganismes): Promise<AnyBulkWriteOperation<IOrganismeInternal> | null> {
...
async function buildHistoricalOrganismeUpdateOperation(chunk: IOrganismeInternal, importMeta: IImportMetaOrganismes): Promise<AnyBulkWriteOperation<IOrganismeInternal> | null> {
    ...
    new Transform({
      ...
      async transform(chunk: ISourceReferentiel, _encoding, callback) {
        try {
          ...
          callback(withCause(internal("import.organismes: error when building organisme", { chunk, importMeta }), error))
        }
      },
    }),
    ...
    new Transform({
      ...
      async transform(chunk: IOrganismeInternal, _encoding, callback) {
        try {
          ...
          callback(withCause(internal("import.organismes: error when building historical organisme", { chunk, importMeta }), error))
        }
      },
    }),
    createBatchTransformStream({ size: 100 }),
