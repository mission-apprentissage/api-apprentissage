...
function getHeaders(values: Value[], columns: ReadonlyArray<ColumnSpec> | "auto", throwInvalid: (d: { extra: string[]; missing: string[] }) => never): Array<string | null> {
  if (columns === "auto") {
...
async function* parseSheet(sheetSpec: ExcelParseSheetSpecRequiredOrOptional, worksheetReader: ExcelJs.stream.xlsx.WorksheetReader): AsyncGenerator<ExcelParsedRow, void, void> {
  let hasData = false
  ...
  try {
    for await (const row of worksheetReader) {
      ...
      if (headers === null) {
        ...
        headers = getHeaders(getCells(row).map(parseCell), sheetSpec.columns, ({ extra, missing }: { extra: string[]; missing: string[] }) => {
...
function getParseSheetSpec(unprocessedSpecs: Set<ExcelParseSheetSpec>, worksheetReader: ExcelJs.stream.xlsx.WorksheetReader): ExcelParseSheetSpec | null {
...
async function* parseWorkbook(parseSpec: ExcelParseSpec, workbookReader: ExcelJs.stream.xlsx.WorkbookReader): AsyncGenerator<ExcelParsedRow, void, void> {
...
export function parseExcelFileStream(readStream: Stream, parseSpec: ExcelParseSpec): AsyncGenerator<ExcelParsedRow, void, void> {
