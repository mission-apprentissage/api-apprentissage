...
export async function scrapeRessourceNPEC(): Promise<{ url: string; date: Date; title: string; description: string }[]> {
  ...
  while (page <= maxPage) {
    const raw = await client.get<string>(`?thematics=financement-de-la-formation-professionnelle-et-de-lapprentissage&group=referentiels&paged=${page}`, {
