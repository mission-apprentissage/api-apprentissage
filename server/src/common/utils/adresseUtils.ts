...
export const getDepartementCodeFromCodeInsee = (codeInsee: string) => {
  const code_dept = codeInsee.substring(0, 2);
  return ["97", "98"].includes(code_dept) ? codeInsee.substring(0, 3) : code_dept;
};

export const buildAdresseFromApiEntreprise = async (siret: string) => {
  const etablissementApiInfo: ApiEntEtablissement = await getEtablissementDiffusible(siret);
...
export const buildAdresse = (adresse: any) => {
