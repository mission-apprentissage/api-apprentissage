...
describe("PUT /job/v1/offer/:id", () => {
  ...
  it("should support full model", async () => {
    ...
    vi.mocked(updateJobOfferLba).mockResolvedValue();
    ...
    expect(updateJobOfferLba).toHaveBeenCalledOnce();

    expect(updateJobOfferLba).toHaveBeenCalledWith(
      "1234",
      expectedLbaOffer,
      expect.objectContaining({
        _id: userWrite._id,
  ...
  it.each<[number, IResError, (messageOrError?: string | Error, data?: unknown) => Boom<unknown>]>([
    ...
    vi.mocked(updateJobOfferLba).mockRejectedValue(errorBuilder(expectedError.message, expectedError.data));
  ...
  it("should throw back 500 without details", async () => {
    ...
    vi.mocked(updateJobOfferLba).mockRejectedValue(internal("Some sensitive message", sensitiveData));
