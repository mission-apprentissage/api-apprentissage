...
import type { ISourceFranceCompetence, ISourceFranceCompetenceDataPart } from "shared/models/source/france_competence/source.france_competence.model"
...
async function buildContinuityMap(importMeta: IImportMetaFranceCompetence): Promise<Map<string, Map<string, { fromAncien: boolean; fromNouveau: boolean }>>> {
...
async function indicateurContinuity(importMeta: IImportMetaFranceCompetence): Promise<{ anciens: number; nouveaux: number }> {
...
async function getUnprocessedImportMeta(dataset: IDataGouvDataset): Promise<IImportMetaFranceCompetence[]> {
  try {
    const successfullyOrPendingImported = await getDbCollection("import.meta")
      .find<IImportMetaFranceCompetence>({ type: "france_competence", status: { $ne: "failed" } }, { sort: { import_date: -1 } })
      .toArray()

    const successfullyOrPendingByResourceId = successfullyOrPendingImported.reduce<Map<string, IImportMetaFranceCompetence>>((acc, meta) => {
      if (meta.status === "done") {
    ...
    return dataset.resources.reduce<IImportMetaFranceCompetence[]>((acc, resource) => {
      ...
      const lastSuccessfullyOrPendingTime = successfullyOrPendingByResourceId.get(resource.id)?.archiveMeta.last_updated.getTime() ?? 0
