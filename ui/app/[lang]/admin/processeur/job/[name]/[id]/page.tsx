...
export default function JobInstancePage({ params }: PropsWithLangParams<{ name: string; id: string }>) {
  ...
  return (
    <Box>
      <Breadcrumb pages={[PAGES.static.adminProcessor, PAGES.dynamic.adminProcessorJob(name), PAGES.dynamic.adminProcessorJobInstance({ name, id })]} lang={lang} t={t} />
      <Typography variant="h2" gutterBottom>
        {PAGES.dynamic.adminProcessorJobInstance({ name, id }).getTitle(lang, t)}
      </Typography>
      <ProcessorStatusProvider>
        {(status) => (
          <ProcessorStatusTaskComponent name={name} status={status} baseUrl={new URL(PAGES.static.adminProcessor.getPath(lang), publicConfig.baseUrl).href} id={id} type="job" />
        )}
      </ProcessorStatusProvider>
    </Box>
  )
